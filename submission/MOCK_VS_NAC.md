...
> **Updated 9 September 2026.** Since this document was written, `/judge` gained
> a judge-selectable **Evidence source**: *Mock — Nokia-compatible demo*
> (default) or *Real NaC — Nokia hosted simulator*, chosen per run on the same
> page. Everything below about the mock's fidelity still holds and still
> describes the authored fixture provider. Two things are new and are described
> where they belong rather than folded in here:
>
> * A second, narrower mock that answers the **real Nokia SDK over a local HTTP
>   transport**, so the request under test is the one the SDK actually
>   serializes. Its coverage is exactly two operations and two subscribers, and
>   the exact scope is in [`../docs/NAC_MOCK_PARITY.md`](../docs/NAC_MOCK_PARITY.md).
> * A real-call option, deployed and checked on the public app on 9 September
>   2026. Two browser runs made four successful Nokia requests, with Gemini
>   selections and verified receipts. See the
>   [deployed observation](../docs/nac/observations/2026-09-09-deployed-judge-smoke.json).
>
> The three authored stories on `/judge` — the SIM replacement, the clean
> checkout, the unresolved-evidence case — are now grouped and labelled
> **Custom mock scenarios**, because they have no Nokia simulator scenario and
> are available in mock only. That is what the rest of this document is about.
...
4. **A clean clone should work without server credentials.** The hosted real
   path uses private Space Secrets and a bounded organizer-code exchange. The
   repository contains no private signing, Nokia or Gemini key.

Recorded observations include 14 authenticated calls to Nokia's hosted simulator
on 6 September, failures included, five local application runs on 9 September,
and two later runs from the deployed judge page. These are separately dated in
[`docs/nac/observations/`](../docs/nac/observations/). The judge can now demonstrate
the paired real path after the organizer grants access; mock remains the default.
...
That table is about the **deployment-wide** provider. The judge's evidence
source is a different, narrower thing and does not use it: it is chosen per run
by the judge, authorized per session, bounded per run, and it can reach Nokia
even while `ISNAD_PROVIDER=mock`. That is deliberate — it is a separate guarded
execution path with its own startup validation, not a hole in this one. See
[`../docs/NAC_SIMULATOR_SETUP.md`](../docs/NAC_SIMULATOR_SETUP.md).
