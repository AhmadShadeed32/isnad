...
> **Updated 9 September 2026 — four counts, not one.** This document describes
> the APIs the **adapter integrates**. Three other numbers are smaller and must
> not be conflated with it:
>
> | Count | Value | What it means |
> | --- | --- | --- |
> | Adapters implemented | 8 | Actions `app/providers/nac.py` can perform |
> | Exposed in the judge's paired evidence demonstration | **2** | SIM Swap `check`, Device Swap `check` |
> | With verified local mock parity | **2** | The same two — [`../docs/NAC_MOCK_PARITY.md`](../docs/NAC_MOCK_PARITY.md) |
> | Observed answering this account from Nokia | see the per-API table below | Dated observations, per operation |
>
> The judge's real evidence source, added 9 September 2026, buys **only** those
> two checks and caps a run at two outbound attempts. After local validation,
> it was deployed to the Space and both scenarios were exercised there: four
> Nokia HTTP 200 responses, with Gemini selections and signed receipts. See the
...
slow or failed verification, so `/console` shows it in a panel that states in the
...
- **Any Nokia congestion subscription create, get, query or delete**, and any
  Nokia callback delivery. `congestion_list` returned 200 with an empty
  collection — the account can read the collection and nothing is leaked.
  The deployed Space now has a reachable server-owned callback base, and its
  mock lifecycle passed on 9 September, but the configured console provider was
  `mock` and sent nothing to Nokia. The probe still refuses an unowned callback
  destination.
