...
def test_a_date_never_moves_the_score():
    ...
    # are units not available for another check. Pinned so a future change to
    # the enrichment price shows up as a test failure instead of silently
    # rebalancing what the agent can afford to look at.
    #
    # These numbers were 7 vs 8, across chains of 3 and 4 links, until the
    # investigator stopped selecting Location Verification for a request that
    # carries no `claimed_location`. The fourth link the un-enriched run used
    # to buy with its spare budget WAS that location check, and with no claim
    # to answer it could only ever return EVIDENCE_UNAVAILABLE — 3 units for a
    # guaranteed hole in the chain. Both runs now gather the same three links,
    # and the date's cost shows up as cost rather than as a check displaced by
    # one that could not answer. See docs/_internal/HACKATHON_FINISHING_RECORD.md.
    assert len(with_dates.chain) == len(without.chain)
    assert with_dates.evidence_cost == 7.0
    assert without.evidence_cost == 5.0
    # The whole difference is the two date calls, and nothing else moved.
    assert with_dates.operations == without.operations + 2
