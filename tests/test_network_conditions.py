...
def test_the_panel_is_a_separate_section_from_the_verdict():
    """On /console since 8 September 2026.

    The property under test is unchanged and is the reason it moved: this is
    information about the NETWORK, kept in its own section, away from the
    decision. It used to sit on /judge, where it competed with the verdict for
    attention and pushed the primary action below the first screen. Its routes,
    its isolation and its subscription state are the same; only its address
    changed.
    """
    from pathlib import Path

    page = read_ui_source(Path("app/static/console.html"))

    assert 'class="network-conditions"' in page
    assert "ui.network_conditions_scope" in page


def test_the_panel_is_not_on_the_checkout_page_any_more():
    """The other half of the move. Markup removed without its listeners would
    have thrown at module load and taken the whole judge page with it."""
    from pathlib import Path

    judge = read_ui_source(Path("app/static/judge.html"))

    # Markup, element ids and route calls — not the word, which still appears
    # in the comments that record where the panel went and why.
    assert 'class="network-conditions"' not in judge
    assert "ncSubscribe" not in judge
    assert "ncLevelNode" not in judge
    assert "/v1/network-conditions/subscriptions" not in judge
...
def test_the_panel_never_requests_anything_on_load():
    ...
    page = read_ui_source(Path("app/static/console.html"))
    # The panel is the last block in console.js, so it runs to the end of the
    # source rather than up to a following function.
    panel = page[page.index("--- network conditions") :]
