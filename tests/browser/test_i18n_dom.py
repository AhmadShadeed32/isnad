...
def test_a_result_rendered_in_english_becomes_arabic_when_the_reader_switches(judge, problems):
    ...
    english = judge.locator(".network-conditions .nc-state").inner_text()

    switch(judge, "ar")
    arabic = judge.locator(".network-conditions .nc-state").inner_text()

    assert english != arabic
    assert judge.locator("#ncTitle").inner_text() == "أحوال الشبكة"
    assert problems == []
...
def test_a_failed_dictionary_leaves_the_page_english_and_working(page, server):
    ...
    assert "أحوال" not in page.locator("#ncTitle").inner_text()
    assert page.locator("#ncTitle").inner_text().strip() != ""
