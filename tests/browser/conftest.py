...
def server_extra_env() -> dict[str, str]:
    """Per-file overrides for the isolated server's environment.

    Empty by default, so every existing file gets exactly the environment it
    got before. A file that needs a different setting — a small replay page
    size, say, so paging is exercised rather than asserted about — overrides
    this fixture in its own module instead of starting a second server.
    """
    return {}


@pytest.fixture(scope="module")
def server(tmp_path_factory, server_extra_env):
