...
def test_reverse_endpoint_maps_trust_vocabulary():
    ...
    # This used to assert "Bank of Jordan" in body["reason"], i.e. that
    # caller-supplied text was spliced into the prose. That is the S11 finding,
    # not a requirement: it meant the reason shown to a user was not the reason
    # the vault signed. The identity is returned in its own field, and the
    # reason is now the signed one verbatim.
