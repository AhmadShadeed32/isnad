...
The public Hugging Face demo defaults to mock evidence and offers bounded Nokia
hosted-simulator swap checks after an organizer-code exchange. It is not the
live handset target. A supported handset and a separately authenticated
deployment with its operator callback registered are still required.
