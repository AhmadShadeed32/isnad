...
| Retries | **The SDK retries twice by default** — three attempts on 408/429/500/502/503, with a sleeping backoff, measured in `test_the_sdk_retries_by_default_which_is_why_every_call_site_overrides_it`. Three billable calls where a signed chain claims one. Every call site therefore passes `max_retries: 0` explicitly: `app/providers/nac.py::_bounded()` and `scripts/nac_demo_probe.py::REQUEST_OPTIONS`. A 422 is not retried. | Measured 2026-09-06 |
...
| Number recycling | `number_recycling.check` | `POST passthrough/camara/v1/number-recycling/number-recycling/v0.2/check` | `phoneNumber`, `specifiedDate` (date, **required**) | `phoneNumberRecycled` | **Entitled** (200 observed) | — | 2026-09-06 hosted: `…1000` with reference 2026-01-15 → `phoneNumberRecycled: true`; `…1001` → `false`; **a future reference date (2030-01-15) → 400**. | Path is **v0.2**. `specifiedDate` must be a genuine merchant-held last-verified date, not a backend guess. An out-of-range date is an operator **error**, not a `false` — so Isnad refuses one locally rather than spending a call to be told. |
...
Reconciled 9 September 2026. Two entries below used to contradict the dated
observations in the tables above — they asked whether operations were entitled
whose 200 responses are recorded on this same page. An unknown that the record
already answers is not caution, it is an unread record; and generalising a
single account's observation into "entitled" would be the opposite error. Both
are now stated as what was actually observed, by account, environment, operation
and date.

- Whether the SDK default host answers (only the catalog host was observed).
  Unchanged. The judge's real evidence path therefore selects the catalog host
  explicitly (`ISNAD_JUDGE_NAC_BASE_URL`), and
  `tests/test_judge_evidence_modes.py` asserts the outgoing URL rather than
...
- **Resolved, and narrowed.** Number recycling and call forwarding returned 200
  from this account, in the Simulator environment, on 2026-09-06 — the responses
  are in `docs/nac/observations/2026-09-06-hosted-simulator.jsonl` and in the
  operation rows above. That is an observation about **those operations, on that
  account, in that environment, on that date**. It is not production
  entitlement, not a statement about any other account, and not a promise that
  the same call answers tomorrow. Consent Info and KYC tenure remain genuinely
  unobserved: **none**.
- **Consent Info, corrected placement.** The capture URL Consent Info can return
  is a **top-level, optional** field on the response, alongside the per-scope
  `statusInfo[]` entries — not a property of a scope. It reports where consent
  *could* be captured; it does not grant consent, and following it does not
  produce a valid consent without a subsequent validity check. Treated as
  untrusted input: it is never fetched server-side, never placed in a planner
  prompt, and never rendered into a public trace.
- Whether the authored *new SIM, same handset* story has any supported simulator
  ...
  neither. That case stays a visibly labelled local fixture, and on `/judge` it
  now sits in a separate **Custom mock scenarios** group that cannot be selected
  as a real Nokia run.

## What the judge demonstration exposes from this matrix

Added 9 September 2026. This page lists every operation the **adapter**
implements. The judge's paired evidence selector exposes exactly two of them —
`sim_swap.check` and `device_swap.check` — because those are the two whose mock
contract, real adapter and scenario coverage are all established. See
[`NAC_MOCK_PARITY.md`](NAC_MOCK_PARITY.md). Implemented adapters, exposed
operations, operations with verified mock parity, and observed hosted calls are
four different counts and must never be reported as one.
