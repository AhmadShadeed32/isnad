...
from app.api.deps import key_from_bearer, owner_for, require_read_access
...
from app.judge.access import SESSION_HEADER as JUDGE_SESSION_HEADER
...
async def console_mode(_owner: str = Depends(require_read_access)) -> dict:
    """What the console needs to label the run honestly.

    Behind a credential because /health used to answer this to anyone (S13):
    whether calls cost money, and whether the demo write endpoints are live, are
    both things an attacker wants and a stranger should not get.

    A judge session counts as one. Outside demo mode the judge page carries no
    demo token at all, so this route answered 401 — and `judge.js` treats a
    failure here as fatal and stops before connecting the event stream. The
    result was a production-posture judge who got a decision with none of the
    reasoning the page exists to show. Nothing here is per-tenant or billable;
    it is the same four configuration facts either way.
    """
    return {
...
async def console_stream_token(_owner: str = Depends(require_read_access)) -> dict:
    ...
    """
    try:
        token = stream_token.mint(_owner)
...
async def replay_run_events(
    ...
    through the caller's own credential, before any row is read — never
    derived from the run_id itself, which a client could simply guess or copy
    from another tenant. A judge session is accepted here and resolves to its
    OWN owner, so recovering a judge's journal never reaches another judge's.
...
async def console_run(
    ...
    authorization: str = Header(default=""),
    judge_session: str = Header(default="", alias=JUDGE_SESSION_HEADER),
) -> dict:
    ...
    # A judge drives these authored stories from the same page as the paired
    # Nokia sources, and must own what they produce. Without this the custom
    # stories would be written under the shared console tenant while the
    # judge's stream, journal and receipts are their own — so a judge's own
    # trace would arrive on somebody else's stream.
    judge = judge_access.get(judge_session.strip() or None)
