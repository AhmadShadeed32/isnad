...
class Belief:
    ...
    def explain(
        ...
        *,
        ...
        """
        # Named before the generic unresolved sentence, and before the DECLINE
        # branch, because it is the more specific true statement: the agent is
        # not unsure about this customer, it was unable to finish the one check
        # that would have settled the question. Saying "insufficient network
        # evidence" here would describe a hole somewhere in the chain; saying
        # "fraud indicators" would describe a finding that was never confirmed.
        if uncorroborated:
            return (
                "Adverse signal not corroborated: "
                + ", ".join(sorted(uncorroborated))
                + " could not be confirmed or ruled out by the checks policy "
                "requires before it may decline on its own."
            )
        if unresolved:
