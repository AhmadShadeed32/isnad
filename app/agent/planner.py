...
class LLMPlanner:
    ...
    def __init__(self, engine: PolicyEngine, *, max_calls: int | None = None):
        self.engine = engine
        ...
        self._calls = 0
        # A caller may tighten the per-investigation ceiling below the
        # deployment's own. The guarded judge path does, so a demonstration
        # cannot spend more of the deployment's model quota than it declared.
        # It may only tighten: `min` here means a caller cannot raise it.
        self._max_calls = (
            settings.llm_max_calls_per_investigation
            if max_calls is None
            else min(max_calls, settings.llm_max_calls_per_investigation)
        )
