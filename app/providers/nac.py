...
class NacProvider:
    ...
    # Class-level defaults so an instance built without __init__ — the
    # `object.__new__` shortcut tests/test_nac_adapter_wire.py uses to drive the
    # adapter over a mock transport — still has honest provenance rather than an
    # AttributeError. An instance that really did call Nokia overrides them.
    source = "nac"
    environment = "nokia_hosted_simulator"
    contract_version = ""
    ...
    def __init__(
        self,
        number_verification_token: str | None = None,
        *,
        httpx_client: httpx.Client | None = None,
        api_key: str | None = None,
        source: str = "nac",
        environment: str = "nokia_hosted_simulator",
        contract_version: str = "",
        base_url: str | None = None,
    ) -> None:
        """Build the adapter, optionally over a caller-supplied transport.

        `httpx_client` is the seam the judge's mock evidence source uses. It is
        a normal constructor argument on purpose: `tests/test_nac_adapter_wire.py`
        reaches the same place through `object.__new__`, and that shortcut is a
        test's licence, not a runtime design.

        `base_url` selects among the allowlisted gateways for this instance
        only. It is a choice between two recorded Nokia hosts, never a
        caller-supplied endpoint.

        `source` and `environment` are the provenance this instance stamps on
        every link it produces. They are parameters rather than the constant
        this class used to hardcode, because the same SDK serializer answering a
        local transport must not be able to leave `nac` on an answer that never
        left the process.
        """
        # A caller-supplied transport brings its own credential: mock evidence
        # must work on a machine that has never held a Nokia key, and must never
        # be able to spend a real one.
        key = api_key if httpx_client is not None else settings.nac_api_key
        if not key:
        ...
        if api_class is not None:
            extra = {"httpx_client": httpx_client} if httpx_client is not None else {}
            self.client = api_class(
                rapidapi_host=settings.nac_rapidapi_host,
                api_key=key,
                base_url=base_url or settings.nac_base_url,
                **extra,
            )
            return
        ...
        if client_class is not None:
            self.client = client_class(token=key)
            return
