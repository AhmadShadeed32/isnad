...
class Settings(BaseSettings):
    ...
    # --- Judge-selectable evidence source (the paired Nokia demonstration) ---
    #
    # A judge on /judge picks Mock or Real NaC per run. Mock needs nothing here
    # and is always available. Real is a NEW guarded execution path: it can
    # spend Nokia allowance even when ISNAD_PROVIDER is mock, so it has its own
    # startup validation rather than borrowing the default provider's.
    judge_real_nac_enabled: bool = False
    # The judge's real path uses the host that has actually been observed to
    # answer this account (eleven authenticated calls on 2026-09-06, recorded in
    # docs/nac/observations/). The application-wide default is deliberately left
    # on the SDK's own host, because that one is still untested and changing it
    # would change the `ISNAD_PROVIDER=nac` path on an assumption. Same
    # allowlist: a documentation URL is not an API base URL, and no caller may
    # supply one.
    judge_nac_base_url: Literal[
        "https://network-as-code.p-eu.rapidapi.com",
        "https://network-as-code.p-eu.apihub.nokia.io",
    ] = "https://network-as-code.p-eu.apihub.nokia.io"
    # Organizer-issued codes that grant a judge session its bounded NaC
    # capability. Comma-separated, server-held, never rendered into a page. An
    # ordinary public demo token must never confer this capability, which is
    # why the capability is a separate exchange and not a property of the page.
    judge_access_codes: str = ""
    judge_session_ttl_seconds: int = 1800
    # A merchant key a judge mints behind the same code. Longer than the
    # session that minted it: the key is copied into a terminal and used
    # after the page is gone. Only issued where the default provider is the
    # mock, so the ceiling is on memory and tidiness rather than on money.
    judge_api_key_ttl_seconds: int = 7200
    # Starting caps, not an assertion about price or entitlement. Every one of
    # them is counted at the transport, below the agent's own choreography.
    judge_nac_attempts_per_run: int = 2
    judge_nac_attempts_per_session: int = 6
    judge_nac_attempts_per_hour: int = 24  # deployment-wide, rolling hour
    judge_gemini_calls_per_run: int = 3
    judge_max_concurrent_real_runs: int = 2
    ...
    @property
    def judge_codes(self) -> set[str]:
        return {c.strip() for c in self.judge_access_codes.split(",") if c.strip()}
...
def check_judge_capability(cfg: Settings) -> None:
    """Validate the guarded judge NaC path on its own terms.

    This is deliberately NOT folded into `makes_billable_calls`. That predicate
    answers "does the DEFAULT provider reach the network", and the judge path
    is a second, separately authorized execution path: it can spend Nokia
    allowance while `ISNAD_PROVIDER=mock`, and it must not silently make every
    public verification route billable to do so.

    The R12 guard it must not weaken is the one that stops a public demo token
    authorizing spend. It is preserved by construction rather than by
    configuration: the NaC capability is granted only by exchanging an
    organizer-issued access code, and a demo token can never carry it. That is
    asserted in tests/test_judge_evidence_modes.py, not assumed here.
    """
    if not cfg.judge_real_nac_enabled:
        return
    if not cfg.nac_api_key:
        raise InsecureConfiguration(
            "ISNAD_JUDGE_REAL_NAC_ENABLED=true but ISNAD_NAC_API_KEY is empty. The "
            "judge's real evidence source would offer a choice that cannot run."
        )
    codes = cfg.judge_codes
    if not codes:
        raise InsecureConfiguration(
            "ISNAD_JUDGE_REAL_NAC_ENABLED=true but ISNAD_JUDGE_ACCESS_CODES is empty. "
            "Real NaC runs require an organizer-issued access code; with none "
            "configured the capability could only be granted to everyone or to "
            "no one. Generate one: "
            "python3 -c 'import secrets; print(secrets.token_urlsafe(16))'"
        )
    if PUBLISHED_DEMO_KEY in codes or any(len(code) < 12 for code in codes):
        raise InsecureConfiguration(
            "ISNAD_JUDGE_ACCESS_CODES contains a published or guessable value. A "
            "judge access code authorizes real Nokia spend; it must be at least "
            "12 characters and must not be a value that appears in the README."
        )
    if not cfg.subject_pepper:
        raise InsecureConfiguration(
            "ISNAD_SUBJECT_PEPPER is unset while the judge's real NaC path is "
            "enabled. Signed chains bind to an HMAC of the subject under this "
            "pepper (S5), and a judge's receipt must survive a restart."
        )
    from app.chain.vault import resolve_key_path
    ...
    if key_path is None or not key_path.exists():
        raise InsecureConfiguration(
            f"No vault signing key at {key_path} while the judge's real NaC path "
            "is enabled. Starting would generate a fresh one and every receipt "
            "issued before this restart would report invalid (S6)."
        )
    limits = {
        "ISNAD_JUDGE_NAC_ATTEMPTS_PER_RUN": cfg.judge_nac_attempts_per_run,
        "ISNAD_JUDGE_NAC_ATTEMPTS_PER_SESSION": cfg.judge_nac_attempts_per_session,
        "ISNAD_JUDGE_NAC_ATTEMPTS_PER_HOUR": cfg.judge_nac_attempts_per_hour,
        "ISNAD_JUDGE_GEMINI_CALLS_PER_RUN": cfg.judge_gemini_calls_per_run,
        "ISNAD_JUDGE_MAX_CONCURRENT_REAL_RUNS": cfg.judge_max_concurrent_real_runs,
    }
    for name, value in limits.items():
        if value <= 0:
            raise InsecureConfiguration(
                f"{name} must be a positive allowance while the judge's real NaC "
                f"path is enabled; got {value}. An unbounded or zero allowance is "
                "not a limit, it is an unstated one."
            )
