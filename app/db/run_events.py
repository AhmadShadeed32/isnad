...
_ALLOWED_BODY_KEYS = frozenset(
    {
        ...
        "calls",
        "threshold",
        # Swap-date enrichment (`app/agent/investigator.py::_emit_timing`).
        # Bounded structured values only: an availability enum and an
        # operation name from app/timing.py's own vocabulary, an operator
        # timestamp, two numbers and a boolean. Without them a recovered
        # `enrichment` row rendered as an empty "Operator change date"
        # heading with nothing under it — the row said a separate call
        # happened and then could not say what it returned.
        #
        # `provider_time` is an ISO timestamp the operator reported, not
        # prose: it is the one fact the row exists to show, and the page
        # renders it verbatim rather than describing it. `reason` is already
        # allowlisted above and carries app/timing.py's authored fallback
        # sentence for the unavailable states — never a provider string.
        "availability",
        "operation",
        "provider_time",
        "age_seconds",
        "monitored_period_days",
        "disagrees_with_window",
    }
)
